...
| 封面 | ✅ |
| 自主声明 | ✅ probe 9 验证通过 |
| 定时发布 | ✅ probe 9 验证通过(设 now+3h,抖音显示 08:00「继续编辑」= 待发布) |
| profile 抓取(昵称/头像) | ✅ |
...
> **2026-08-08 更新**:A1 自主声明、A2 定时发布已验证通过(probe 9),
> 剩下 A3 合集、A4 多账号、A5 公开发布。

### ~~A1. 自主声明~~ ✅ 已验证(2026-08-08, probe 9)

用 `self_declaration='虚构演绎，仅供娱乐'` 真实发布成功。此前两次失败确实是
被封面弹窗挡住,选择器本身一直是好的 —— #1739 修完立刻通。
...
### ~~A2. 定时发布~~ ✅ 已验证(2026-08-08, probe 9)

设 `now+3h`,抖音作品卡显示 **08:00** 且按钮是**「继续编辑」**(其他作品是
「编辑作品」)—— 待发布状态,定时真的生效了。注意 `success` 表示**提交成功**
(已排入定时),不是已发出,这个语义是对的。
