...
- **Infra-vs-fault reason enum.** `lost`/`worker_lost` (→ recover/retry, surface "interrupted") vs `failed` + decoded real error (→ fault). Propagate the real error, never overwrite with a generic "lost" string. (hermes `completion_reason: lost|exited`, nous #668.)
