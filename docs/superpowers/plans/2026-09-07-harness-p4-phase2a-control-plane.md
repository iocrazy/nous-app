...
- 循环顶新增 `paused_at` 检查（计划没写）；`/resume` 先清 `paused_at` 再派发、失败恢复；`/resume` 撞上未观察到的 pause 请求时撤回而不派发（`AgentRunsRepository.clear_pause_request`）；`request_pause` 的 `user_id` 可选（目标层鉴权）；`resumed_from_run_id` 写 `execution_state`（计划口径），spec §2 原文的 `metadata_json` 落点不可行（run 行尚不存在）。
...
- 暂停中 `answer_to` → 409 `issue_paused`（spec §2 已回写理由）。
- 「暂停中 cancel 清 `paused_at`」放在 `issue_repository.transition_status`。
