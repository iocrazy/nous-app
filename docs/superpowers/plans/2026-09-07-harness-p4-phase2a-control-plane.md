...
| 前端 | 类型化提问卡（修复上线后补验，2026-09-08 晚） | ✅ | MH-72（AskUser）详情页 `question-card` 出两个 `question-option` 按钮「Option A: Comedy / Option B: Thriller」，页面点击 A → 按钮锁定、无 `question-error`，后端提问 run 落 `question_answered{value:"Option A: Comedy"}`、新 run 跑完 → `in_review`；MH-73（零预算）Settings → Task Center「Needs your answer」出 `Pick one of 3` + Top up / Wrap up / Cancel，页面点 Cancel → 该行消失，后端 issue `cancelled`、`outcome_reason=budget_exhausted`。截图 06–10 在会话 scratchpad。 |
