...
**Files:**
- Create: `supabase/migrations/423_preset_doubao_fallback.sql`（号以 fetch 复核为准；写计划时 master 水位 420）
...
- [ ] **Step 4: Commit** `git commit -m "chore(migrations): 三个 doubao 预设 agent 配 fallback — pro→lite,只填空数组 (mig 423)"`
