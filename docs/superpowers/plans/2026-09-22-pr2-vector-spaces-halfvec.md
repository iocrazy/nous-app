...
- 迁移编号 **499**（490 = #2396、491 已合并、492 = #2398、493 = #2399、494 = #2401、495 = #2402、496 = #2403 都已合并）。合并前再核一次 `ls supabase/migrations | tail`。
...
```sql
-- 499: resource_embeddings (halfvec 2048 + HNSW) + embedding_spaces.
--
...
```python
"""embedding_spaces / resource_embeddings (migration 499).
    ...
    async def get_or_create(self, spec: SpaceSpec) -> dict:
        ...
        try:
            ...
            if _is_missing_relation(exc):
                raise EmbeddingStoreMissing("embedding_spaces does not exist (migration 499)") from exc
            raise
...
- [ ] **Step 9: Commit** `feat(db): embedding_spaces + resource_embeddings（halfvec 2048 + HNSW）与两个 repository（mig 499）`
...
- CLAUDE.md 向量陷阱条末尾加：`**2026-09-23 起（mig 499）语义层向量在 resource_embeddings（halfvec(2048) + HNSW，按 space_id 分空间）；resource_analysis.content_embedding 只读回退、下一版删。换模型 = 新空间 + 全量重嵌（回填端点就地嵌，不再派 VLM）。`
