...
> **实施记录（2026-09-10，Task 7 真栈验收与 7a/7b/7c 修复）**
> - §1：`inflight_count` 无 healthz 消费方，`workforce_router` 健康端点此前读无人设置的 `app.state.workforce_scheduler` 恒报 down，改读 DBOS launched + 派生 inflight + `task_tracking` 派发停摆信号；`InboxProcessor` 的 dispatcher 参数删除；workflow id 带派发尝试号 `workforce-<task>-<attempt>`（同 id 二次 enqueue 在 DBOS 只 bump `recovery_attempts`、`should_execute=False`）；`claim_task` 两臂（`queued` 首次 / 同 workflow id 的 `assigned|in_progress` 再入）；缺 workflow id 类型化拒绝；集成套件挂 `schema-drift.yml`（`user_id` fixture 从 skip 改播种，否则门禁在空 `auth.users` 库上空过）。心跳/stale worker 清扫在生产零接线 → 小票。
> - §2：子代理/Delegate 的父 run 必须在 spawn/execute 时从**活动 recorder** 解析（wiring 传的是祖先 id，根 issue run 上为 None；根 run 上 Delegate 环检测因此一直关闭）；`_spawn(await=False)` 只建 `queued` 行由 tick 派发；worker 的 `subagent` 分支在 `agent.persistent` 门之前且**只回报** `idle_dispatch`，由 workforce workflow body 派发（`dedupe_key=subagent-wake-<task_id>`）；续聊所有权 = 祖先链 **或** 同 issue；envelope 带 `cost_cents`；父 run 有子代理时 mirror/finish 前从持久化 `subagent_*` 事件重折 `children`/`by_child`（并发子完成不丢）；`inbox_claimed` 事件带有界 `content`（`summary`/`text` ≤500、`description` ≤200、`source` 完整、无 `dedupe_key`）；后台子代理继承 `issue_id`（payload + `task_tracking.issue_id`）。子代理结果到达时若 issue 空闲：轮内有界补跑（≤3，`awaiting_input` 时不做）+ sweeper 每分钟空闲兜底（scan step 只读、body 派发；跳过持锁/派发中/暂停/终态 issue；20/tick；过期改 WARNING 不静默）。
> - §3：`payload.run_id` 是字符串（Snowflake 精度）；一次性行 step 只校验并返回 `fire_key=sched:<id>:<next_fire_at>`，body 用 `deliver_or_dispatch(dedupe_key=fire_key)` 投递，`finish_issue_wakeup_step` 成功才标 `fired_once`，失败走 `consecutive_fails`（5 次 auto-pause），只有 `{issue_terminal, issue_missing}` 才禁用；`_disable_schedule` 总写 `paused_at`；`DELETE /schedules/{id}` 返 200 `{ok, deleted}`；非法 task_type code 为 `unsupported_task_type`；唤醒来源写在**会话消息** `metadata_json.source`（线程 API 在 `ai_session_id` 有值时读会话消息；`issue_messages` 镜像行保留），且只追加一次；reply 派发器也写 `dispatching` 标记并由 `acquire_turn_lock` / 终态清除。运行中到点的唤醒文本无线程镜像行 → 小票。
> - §4.1：`resume` 探不到 `issue_busy`（`not_paused` 判定在前），窗口守卫用 fork 端点验证。§4.2：回填在 gpupc 容器内直调模块（admin 端点需 admin 账号），3 行补齐、剩余 0。
> - §5：右栏 Schedules 块对已触发一次性行显示 Fired 无 ✕（暂停的例行仍可删）；agent 自设行紧凑时间；子代理卡 `status` 上屏（failed 为 danger）；摘要行来自同 run 内 `subagent_result` 领取的配对，后台子代理的领取落在后续 run、配对不可达（第三轮复验缺陷 J）→ `subagent_done` 事件带有界 `summary`（与 `inbox_claimed` 同一 500 截断）作兜底，配对值优先；续聊芯片无轮次（事件不带）；子 run 面板父 id 由 `TrajectoryRunContext` 透传；`fmtChildCents` 0/null 显 `—`；Later 弹层解码 `details.code`（真实码集 `fire_at_out_of_range` 等六个）。
> - 生产事实：`ai_agents.persistent` 曾为零（162/163 在基线之下从未重跑、seed_loader 不写列）→ 由 seed frontmatter 声明并每次启动 upsert（summarize / analyze / coordinator），健康端点 0 时报 `no_persistent_agents`；`<available_workers>` 与 Delegate 同开关门控（翻开关 = 全站提示词前缀一次性失效，见 `prompts/README.md`）。
