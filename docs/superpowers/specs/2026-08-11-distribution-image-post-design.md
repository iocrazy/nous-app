...
⚠️ **本节初稿判断错了，2026-08-11 实测后改判（T5 / PR #1798）。原文保留在 git 历史里。**

初稿以为图文的阻塞点是「三处写死 video 形状」（item id 正则 `:92`、公开 URL 模板
`:88`、卡内链接选择器 `:422`）。实测推翻：**那三处对视频卡也从来没有生效过**——
作品管理页**整页 `<a>` 元素 = 0**（§3.4 V17b，两次独立读取一致）。不是"图文换用了
`/note/`"，是这一页压根没有锚点。所以它们不是图文的兼容问题，是一段对两种内容
都死的代码。处置见 T5：**保留不删也不参数化**（`published_url` 的语义是"拿得到
就填"，删了会变成"永远不填"，也让平台哪天开始给链接时没人发现），注释升级为
实测硬数字。

**真正的阻塞点初稿完全没写到**：`read_work_cards` 数的是 **DOM 节点**而不是**作品**。
实测 `[class*="video-card"]` 匹配 **72 个节点 = 12 个作品 × 每个 6 层嵌套**，而
`MAX_CARDS = 24` 截断的是节点 → **只覆盖前 4 个作品**，第 5 个起一律判
`not_found`，也就是对用户说"你的作品被删了"。这是**已经在生产里的错判**（P1-3 上线
即存在，只因当时还没跑过真实定时发布而未暴露）。修法：`outermost_only(sel)` →
`sel:not(sel *)`，72 收敛回 12；该 CSS 形式用真 Playwright 引擎在活页面验过。

状态文案表（`:174-196`）**零改动**是对的：[实测 2026-08-11]（§3.4 V17）图文卡与
视频卡在同一张作品表里混排、用同一批状态词。⚠️ 但"同词"只在 `已发布` 上被证实——
账号里当时没有 审核中/定时中/未通过 的作品，其余状态**仍是推断**，代码注释与
fixture 已如实标注。图文卡的区别在别处：前缀是 `N张` 而非时长，指标是
`划走率 / 文案展开率 / 平均浏览图片数`。

失败方向是安全的这一点仍然成立：选择器失效 → `list_unreadable` → INCONCLUSIVE，
**永不误判成"作品被删了"**。而 `published_url` 长期为空是**平台事实不是 bug**——
`publish_readback.py:32-46` 记录 [实测 2026-08-08]：作品卡既没有 href 也没有 id
属性，列表 XHR 也不返回，三处都查过。
...
**做什么**：⚠️ **实测后本任务的内容与初稿设想不同，见 §1.4 的改判。**
链接模式参数化**撤销**（整页零 `<a>`，参数化一串死选择器还是死的）；
状态词表**零改动**（实测同词）；真正要修的是 `read_work_cards` 把 DOM 节点
当作品数、`MAX_CARDS` 截断导致第 5 个作品起误判 `not_found` 的**生产 bug**。
...
1. 把标题改成一个页面上不存在的串 → verdict 必须是 `not_live`/`not_found`，
   证明匹配是真的在比对而不是恒真。
   ⚠️ **这一条必须配合足够深的作品列表**（多于 `MAX_CARDS ÷ 每作品节点数`）：
   在修复前的旧代码下它**也是绿的** —— 旧代码只读得到前 4 个作品，任何深位
   标题都会"正确地" not_found，而那恰恰就是 bug 本身。不带深度的这条断言
   证明不了任何事
