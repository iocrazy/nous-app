...
| **该表没有任何读端点**——W1 读侧本来就欠着 | 全仓 grep 无 API 读它 |
...
1. **重试计数读侧**：`agent_runs` 详情（TaskDetailModal）拉该 run 的 `llm_retry` 事件
   （新只读端点 `GET /api/v1/ai-library/runs/{run_id}/events?type=...`，分页、
   仅 run 所有者可读——这同时补上 transcript 事件表的第一个读端点，todo/压缩/turn_end
   审计共用）。
