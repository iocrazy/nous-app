...
> **实施记录（2026-09-11，Task 1–7 + 3b/6b 全部合并；真栈验收见 plan Task 8 完成账）**
> - §1 登记口：三处分镜/场次登记**必须在 `caller_scope` 之外**——它以 `authenticated` 角色发布 ambient session，而 `run_deliverables` 只有 service_role 策略，块内登记拿 42501 会 abort 同一事务，把 agent 自己的分镜写一起回滚（评审抓到，单测桩 session 看不见）；登记落在 `screenwriting_tools` 的 `async with` 退出之后（**先提交、再登记**），网关零登记引用由源码守卫钉住。写入点调 `register_deliverable_best_effort`（失败记 ERROR 返 None、`ValueError` 照抛）：图已付费、卡已入库后一次瞬时 DB 失败不该让 agent 重试产两份。场次登记在网关 `apply_element_edit` 而非计划写的 repository 两行（那一层与人手编辑器共用、手里没有 run）。**`script_chapter` 今天无生产者**（两个章节 workflow 只由 HTTP router 派发），管道接通、计数与清单预期恒空。`register_generated_media` 有 11 个调用点而非 5 个，`cost_cents` 无任何调用方填，媒体类花费本期为空。咽喉点守卫扫 `insert(RunDeliverables)` 而非类名（读方走工厂）；repository 集成测试挂 `schema-drift.yml`（本机一次性 pg17 真跑过，第一版即抓到三条 FK 事实）。
> - §2 版本：`view.outputs` 多一个有界 `seen` 簿记键（跨 DBOS 事件重放，簿记不随 views 镜像就每次从零去重）。**`generated_media` 在 3a 恒为 v1**：重新生成写的是新行新 ref_id，同一 ref_id 永远一版——§2.1/§5 假设媒体按同一 ref_id 链版本，与 `generated_media` 的身份模型不符；媒体差异端点与弹层按计划立好但真栈按设计不可达；真要做媒体版本链需另一把键（小票）。diff 按 `created_at`（两个 `now()` = 事务开始时间）把版本映射到 ops 账本行，不是外键；真修是账本行 id 列（小票）。
> - §3 数据：462 只补五列 + 两索引 + 收件箱 dedupe 唯一索引（partial，谓词与 `dedupe_lookup_stmt` 逐字同口径）；`LATEST_MIGRATION` 仍指 461（462 不重写事件白名单，有守卫钉住）。**没有任何门禁比对 ORM 索引**（schema-drift 只管表/列/可空/类型/外键）——小票。
> - §4 接口：三端点挂 `api/__init__.py`；未登记 404 `not_registered`、重建不出的一侧 `available:false` + 类型化 `unavailable_reason`（不回空串）；`output_ref` 多一个码 `output_ref_limit_exceeded`（≤8/消息）；**chat 面板入口拒绝引用**（同一类型化码，经共享助手；第一版守卫只认 dict 而真实入参是 pydantic 对象——「mock 形状 ≠ wire 形状」再犯一次，第二轮修）；**被转进收件箱的评论其附件不渲染成框**（`render_inbox_message` 只渲染正文，`resource_ref`/`asset_ref` 同样，P4 之前的缺口，小票）；Task 3b 给血缘每版补 `issue_key`/`deep_link`，全后端只剩一个 todolist 路径构造器 `issue_links.py`。
> - §5 UI：界面叫 Outputs，块 id `outputs` order 25 与既有 `deliverables` 块并存；`ensureStep` 加坐标查找，deliverable **永不开新步**（唯一例外 run 尚无任何步）否则挂最后一步；右栏行悬停高亮用模块级 store + `useSyncExternalStore`（rail 与 thread 无共同 provider）；缩略图调既有 `generatedMediaCoverUrl`，`resolveMediaUrl` 只管 diff 端点的相对路径（后端相对 URL 在 Cloudflare Pages 上会解析到 Pages 源）；**资源信息面板不挂来源块**（无字段可桥接 generated_media id ↔ resources.id，小票）；「Open Issue」走 `<Link>`（裸 `<a>` 从画布点会整页重载）；血缘请求按对象去重缓存，`not_registered` 缓存整页、其它失败驱逐；同页内 agent 后登记的隐身窗口记小票。顺手修：issue 消息错误解析只读 `detail`（真栈所有类型化拒绝退化成 400 Bad Request）。
> - 测试基建：`tests/workflows` 两文件清 `sys.modules` 不还原（含父包属性）导致的顺序假红已修隔离并加末序身份守卫（删 conftest 全量转红）；`GET /ai-library/agents?slug=` 此前根本没有该形参。
> - **真栈修复波（T8a/T8b/T8c，#2251 #2252 #2253）**：§1 活路径的两条产出通道把自己的 recorder 交给登记口，`for_run` 只留给 DBOS 迟到登记；**隐性契约**——同一 run 的 transcript 由两个 writer 写，`UNIQUE(run_id, seq)` 让 `for_run` 每登记一次都吃掉活 recorder 的下一条事件（3a 之前 workforce 的 `subagent_done` 同病，靠白名单重放兜住），现在 `RunEventWriter.append` 撞 23505 就从 DB 重播计数器并有界重试；`view.outputs` 镜像前从 transcript 重折（与 children 同一次读）。§2 `run_deliverables.seq` 在事件落地后 best-effort 回写。§4 逐对象血缘对 `issue_id ≠` 门控议题的旧版置空 `issue_key`/`deep_link`（刻意比泄露面宽，`issue_id` 保留）；deep_link 有 turn 时追加 `&turn=`；`GET /issues/{id}/messages` 投影 `attachments`（老行 `null`、逐条校验）。§5 线程渲染只读引用 chip；`?step=`（`&turn=`）经 `focusTrajectoryStep` 锚到那一步（先摊开含目标的 run-group，对不上退回 step-only，10 s 静默放弃）；稿二「引用 N 件」= `referenced_outputs` 随 `user` 事件进 transcript，第 1 步下 `References N outputs · pinned to version`；右栏与线程共用一个 `ChildRunContext.Provider`，非 timeline 页签点 Open Run 先切页签；Generated 卡来源行 `· run #末六位 · step n`。Cockpit 产出格只在 run 活着时存在（`current_run` 终态即 null）按设计接受，右栏 Outputs 块是持久视图。
