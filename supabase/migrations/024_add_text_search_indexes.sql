...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_videos_desc_trgm
ON douyin_videos USING gin (video_desc gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_videos_hashtag_trgm
ON douyin_videos USING gin (video_hashtag_name gin_trgm_ops);

-- Create GIN indexes for text search on video_analysis
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_analysis_visual_desc_trgm
ON video_analysis USING gin (visual_description gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_analysis_detected_text_trgm
