...
export default defineConfig({
  ...
  // VITE_FEATURE_CANVAS_NAV is a build-time-only flag read via import.meta.env,
  // so it must be baked in here to expose the Canvas nav entry the e2e flows
  // exercise. (The project detail pane is the ProjectWorkspace shell
  // unconditionally now — the WORKSPACE_V2 / WORKBENCH / AI_SUGGEST flags were
  // retired with the legacy detail surface in PR-18.)
  //
  ...
  webServer: {
    command:
      'VITE_SUPABASE_URL=https://e2e.supabase.co VITE_SUPABASE_ANON_KEY=sb_e2e_anon_key VITE_API_URL=http://localhost:4173 VITE_FEATURE_CANVAS_NAV=true npm run build && npm run preview -- --port 4173 --strictPort',
