...
export const ShortcutsTagsPage: React.FC = () => {
  ...
  // Auto-save selection to Redis whenever it changes
  useEffect(() => {
        ...
        <p className="text-center text-sm">
          {selected.size > 0
            ? <span className="text-green-400 font-medium">
                {lang === 'zh' ? `已选 ${selected.size} 个，点 Done 完成` : `${selected.size} selected — tap Done to finish`}
              </span>
            : <span className="text-zinc-500">
                {lang === 'zh' ? '选择标签后点 Done 完成' : 'Select tags, then tap Done'}
              </span>
          }
        </p>
      </div>
