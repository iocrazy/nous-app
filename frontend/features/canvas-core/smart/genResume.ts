 ...
 * in-flight batch, and reset prompts stranded in queued/running with no
 * batch to recover (their runner died with the previous page).
 */
export async function resumePendingGenerations(): Promise<void> {
  ...
  try {
    ...
    for (const node of store.nodes) {
      if (asObj(node).type !== 'prompt') continue;
