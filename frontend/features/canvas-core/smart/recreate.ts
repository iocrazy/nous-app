...
/** Spawn a generation prompt below ``nodeId``, wire source → prompt, and
 *  select it. ``init`` seeds the attached-composer fields (body / gen /
 *  chosen input image). Returns the new prompt id, or null for an unknown
 *  node. */
export function createPromptFromNode(
  nodeId: string,
  init: PromptInit = {},
): string | null {
  ...
  if (init.source_ref) {
    (prompt.data as { source_ref?: string }).source_ref = init.source_ref;
  }
