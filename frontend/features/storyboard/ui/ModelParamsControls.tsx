...
import { AUTO_REQUEST_ASPECT_RATIO } from '../domain/canvasNodes';
import {
  getModelProvider,
...
export const ModelParamsControls = memo(({
  ...
  const handleProviderClick = (providerId: string, providerLabel: string) => {
    const providerApiKey = (apiKeys[providerId] ?? '').trim();
    if (!providerApiKey) {
      setOpenPanel(null);
      setMissingKeyProviderName(providerLabel);
      return;
    }
    if (providerId !== panelProviderId) {
      const firstModel = imageModels.find((model) => model.providerId === providerId);
      if (firstModel) {
        onModelChange(firstModel.id);
      }
    }
    setPanelProviderId(providerId);
  };
                  ...
                  {providerOptions.map((provider) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={(event) => {
                          event.stopPropagation();
                          handleProviderClick(provider.id, provider.label || provider.name);
                        }}
                      >
            ...
            <>
              ...
              <UiButton
                ...
                onClick={() => {
                  ...
                  setOpenPanel(null);
                  // TODO: open provider settings panel when settings UI is implemented
                  console.warn('[ModelParamsControls] Provider settings dialog not yet available');
                }}
              >
