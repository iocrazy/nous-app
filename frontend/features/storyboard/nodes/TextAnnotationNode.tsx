...
import { CANVAS_NODE_TYPES, type TextAnnotationNodeData } from '../domain/canvasNodes';
...
export const TextAnnotationNode = memo(({
  ...
  const handleMarkdownLinkClick = useCallback((href?: string) => {
    ...
    // Web: open in new tab instead of Tauri openUrl
    ...
    >
      ...
      {selected ? (
        <textarea
          ...
          placeholder={t('node.textAnnotation.placeholder', 'Type your notes here...')}
        ...
        <div className="nodrag nowheel h-full w-full overflow-auto px-1 py-0.5 text-sm leading-6 text-text-dark">
          {content.trim().length > 0 ? (
            ...
            <div className="pt-1 text-text-muted">{t('node.textAnnotation.empty', 'Empty note')}</div>
          )}
        </div>
      )}
    </div>
