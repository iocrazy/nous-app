...
> {
  if (!runtimeDiagnosticsPromise) {
    runtimeDiagnosticsPromise = (async () => {
      ...
      // TODO: migrate — replace Tauri getVersion with package.json version
      appVersion = 'unknown';

      // TODO: migrate — replace Tauri getRuntimeSystemInfo with browser-only detection
      void resolvedOsBuild;
