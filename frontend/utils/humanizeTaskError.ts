...
const PATTERNS: ReadonlyArray<ErrorPattern> = [
  ...
  {
    // The job was routed to the user's own machine (codex / dreamina via the
    // paired nous-codex daemon) and no daemon is connected. Not retried by
    // the backend: nothing changes until the user starts it.
    test: /\[daemon_offline\]/i,
    message: 'Your local daemon is not connected.',
    hint: 'Start nous-codex on your machine (run, or install-service to keep it running), then run again.',
  },
  {
    // The daemon took the job and never answered in time. Deliberately NOT
    // the generic "retry usually works" row below: the job may still be
    // running on the user's machine and upload later, and a blind retry
    // pays for the same generation twice.
    test: /\[daemon_timeout\]/i,
    message: 'Your local daemon did not answer in time.',
