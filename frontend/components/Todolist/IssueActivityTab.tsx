  ...
  return (
    <div className="px-4 py-3 space-y-1">
      {events.map((m) => {
        ...
        return (
          <div key={m.id} className="flex items-center gap-2 px-2 py-1.5 text-[12px] text-zinc-400 border-l-2 border-zinc-800/80">
            ...
            <span className="ml-auto text-[11px] text-zinc-500">{relativeTime(m.created_at)}</span>
          </div>
        );
      })}
    </div>
