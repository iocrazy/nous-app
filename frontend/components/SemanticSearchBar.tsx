...
export const SemanticSearchBar: React.FC<SemanticSearchBarProps> = ({
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <div className="relative flex items-center">
        ...
        <div className="absolute right-3 flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 disabled:bg-ink-700 disabled:text-ink-500 text-white text-sm font-medium rounded-lg transition-colors"
          >
            {isSearching ? (
      ...
      {/* Mode Dropdown */}
      {showModeDropdown && (
        <div className="absolute top-full left-0 mt-2 w-56 bg-ink-900 border border-ink-700 rounded-xl shadow-xl z-50 overflow-hidden">
            ...
            {searchModes.map((mode) => (
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="text-xs text-ink-500">{mode.description}</div>
                </div>
              </button>
            ))}
          </div>
      ...
      {showSuggestions && recentSearches.length > 0 && !query && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-ink-900 border border-ink-700 rounded-xl shadow-xl z-40 overflow-hidden">
          <div className="p-2">
            <div className="px-3 py-1.5 text-xs font-medium text-ink-500 uppercase tracking-wider">
              Recent Searches
            </div>
