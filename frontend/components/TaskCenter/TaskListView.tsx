...
export const TaskListView: React.FC<TaskListViewProps> = ({ groups, selectedTaskId, onSelect }) => {
  if (groups.length === 0) {
    return (
  ...
  return (
    <div className="border-x border-b border-zinc-800/80">
      {groups.map((g) => (
        <GroupSection key={g.key} group={g} selectedTaskId={selectedTaskId} onSelect={onSelect} />
      ))}
    </div>
  );
};
