...
export const CollectionPicker: React.FC<CollectionPickerProps> = ({
  ...
  return (
    <div
      ref={panelRef}
      className="absolute bottom-full right-0 mb-2 w-64 bg-ink-900 border border-ink-800 rounded-xl shadow-2xl z-50 animate-in fade-in slide-in-from-bottom-2 duration-200 overflow-hidden"
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-ink-800">
        <h3 className="font-medium text-ink-200 text-sm">{t('collections.addTo')}</h3>
      ...
      <div className="max-h-[240px] overflow-y-auto p-2 space-y-1">
        {collections.map(collection => (
          <button
            ...
            className="flex items-center justify-between w-full px-3 py-2 rounded-lg hover:bg-ink-800/50 transition-colors group"
          >
            <div className="flex items-center gap-2 min-w-0">
              <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                selectedIds.includes(collection.id)
                  ? 'bg-indigo-500 border-indigo-500'
                  : 'border-ink-600 group-hover:border-ink-500'
              }`}>
            ...
            <span className="text-xs text-ink-600">{collection.videoCount}</span>
          </button>
        ))}
      </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-2 py-1 text-xs text-ink-500 hover:text-ink-300"
                >
                  {t('common.cancel')}
                </button>
                <button
