...
export const ProjectCollectModal: React.FC<ProjectCollectModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-lg mx-4 overflow-hidden shadow-2xl max-h-[80vh] flex flex-col"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {isLoading ? (
            ...
            collections.map((c) => (
              <div key={c.id} className="bg-zinc-800/50 border border-zinc-700/50 rounded-xl px-4 py-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-zinc-200">{c.collection_name}</span>
                ...
                <div className="flex items-center gap-3 text-xs text-zinc-500">
                  <span className={`px-1.5 py-0.5 rounded ${c.is_active ? 'bg-green-500/10 text-green-400' : 'bg-zinc-700 text-zinc-500'}`}>
          ...
          {showCreate ? (
            ...
            <button
              ...
              className="w-full py-2.5 border border-dashed border-zinc-700 hover:border-indigo-500/50 rounded-xl text-sm text-zinc-400 hover:text-indigo-300 transition-colors"
            >
