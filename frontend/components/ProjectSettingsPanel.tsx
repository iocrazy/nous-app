...
export const ProjectSettingsPanel: React.FC<ProjectSettingsPanelProps> = ({
  ...
  // the page flow, no backdrop — not an overlay. Availability/visibility follow
  // the open state so the island column appears/collapses with it.
  useEffect(() => {
    setInfoAvailable(isOpen);
    setInfoVisible(!!(isOpen && infoIslandEl));
    return () => {
      setInfoAvailable(false);
      setInfoVisible(false);
    };
  }, [isOpen, infoIslandEl, setInfoAvailable, setInfoVisible]);

  // Close on outside click — ONLY for the mobile overlay fallback. In island
  // mode the panel is part of the page; clicking the grid must not dismiss it.
  useEffect(() => {
    if (!isOpen || infoIslandEl) return;
      ...
      <div className="p-5 space-y-6 flex-1 min-h-0 overflow-y-auto">
        ...
        <div>
          <div className="flex justify-between mb-2">
            ...
            <span className="text-xs text-ink-500">{name.length}/30</span>
          </div>
          <input
            value={name}
        ...
        <div>
          <div className="flex justify-between mb-2">
            ...
            <span className="text-xs text-ink-500">{announcement.length}/100</span>
          </div>
          <textarea
            value={announcement}
  ...
  // sm+: a sibling island in the page flow (portal into the shell's info slot).
  if (infoIslandEl) return createPortal(panelBody, infoIslandEl);

  // Mobile / non-island fallback: floating card over a backdrop (the info
  // island is display:none below sm — same fallback shape as HotspotInfoPanel).
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end p-4">
      <div className="absolute inset-0 bg-black/40" />
      <div
        ref={panelRef}
        className="relative w-96 max-w-full max-h-[calc(100dvh-2rem)] bg-ink-900 border border-ink-800 rounded-2xl shadow-2xl overflow-hidden"
      >
        {panelBody}
      </div>
    </div>
