...
export const TaskMonitor = ({
  ...
  return (
    <div className="mt-8 bg-ink-950 border border-ink-800 rounded-xl overflow-hidden shadow-2xl animate-in slide-in-from-bottom-2 duration-300">
      {/* Header / Status Bar */}
      <div className="bg-ink-900/80 backdrop-blur-sm border-b border-ink-800 p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
             ...
             <span className="font-mono text-sm text-ink-200 font-semibold uppercase tracking-wider">
               {status}
             </span>
          </div>
          <span className="text-xs font-mono text-ink-500">{progress}%</span>
        </div>
        ...
        {/* Quick Stats Grid */}
        <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-ink-800/50">
           <div className="flex flex-col items-center">
              <span className="text-[10px] text-ink-500 uppercase tracking-widest mb-1">Worker</span>
           ...
           <div className="flex flex-col items-center border-l border-ink-800/50">
              <span className="text-[10px] text-ink-500 uppercase tracking-widest mb-1">Network</span>
              <div className="flex items-center gap-1.5 text-ink-300">
                 <Wifi size={14} className={
                   downloadSpeed || systemStatus?.network.status === 'active' ? 'text-emerald-400' :
                   systemStatus?.network.status === 'error' ? 'text-red-400' : 'text-ink-500'
                 } />
                 <span className="font-mono text-xs font-medium">
           ...
           <div className="flex flex-col items-center border-l border-ink-800/50">
              <span className="text-[10px] text-ink-500 uppercase tracking-widest mb-1">Storage</span>
              <div className="flex items-center gap-1.5 text-ink-300">
      ...
      <div className="bg-[#0c0c0e] p-4 h-48 overflow-y-auto font-mono text-xs space-y-1.5 custom-scrollbar">
        {logs.length === 0 && (
          <div className="text-ink-700 italic">Waiting for task logs...</div>
        )}
        {logs.map((log) => (
          <div key={log.id} className="flex gap-3 hover:bg-white/5 p-0.5 rounded px-2 transition-colors">
            <span className="text-ink-600 flex-shrink-0">[{log.time}]</span>
