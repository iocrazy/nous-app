...
transcript (``question_asked`` with no ``question_answered``); the issue is not
parked on it, so no feed lists it and no answer is expected.
"""
...
def resolve_turn_outcome(result: dict[str, Any]) -> TurnOutcome:
    ...
    declared ``completed``. See the module docstring."""
