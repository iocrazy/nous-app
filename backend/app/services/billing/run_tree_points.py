...
扣费**现在**只发生一次：``RunRecorder._finish`` 在 root 定稿时按整棵树的平台花费
``ceil`` 一次（用户裁定 2026-09-17）。此前是逐 run 的 —— 每条 run 各 ``ceil`` 一次
自身花费，于是一次带委派的回合在 ``point_transactions`` 里是好几行 consume。

**本文件两种口径下都不用改**：它按 ``root_run_id`` 取整棵树的全部流水再合计，
一行与好几行相加得到的是同一个数。界面上的「◇ n」问的是「这次回合扣了我多少」，
答案始终是整棵树的合计。

⚠️ 仍然可能不止一行：root 收工之后才结束的子 run 会自己补扣一次（它不在 root 的
``refold`` 快照里，没人替它收）。所以**不要**把这里简化成「取 root 那一条」——
那正是下面 I2 要修的那个低报。
