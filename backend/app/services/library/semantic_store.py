...
``find_similar_media`` all read here. Since migration 499 the vectors live in
...
* :meth:`SemanticStore.nearest` reads the legacy column only when the new
  ...
  before migration 499) or an embedder that cannot name its space. A
...
class SemanticStore:
    ...
    async def current_space_id(self) -> Optional[int]:
        ...
        one. Raises :class:`EmbeddingStoreMissing` before migration 499."""
    ...
    async def nearest(
        ...
        embedder's space; the legacy RPC answers before migration 499."""
        try:
            ...
            logger.warning(
                "[search] resource_embeddings unavailable (migration 499 not "
    ...
    async def similar(
        ...
        try:
            ...
            logger.warning(
                "[similar] resource_embeddings unavailable (migration 499 not "
                f"applied), reading the legacy column: {e}"
            )
            rows = None
        if rows is not None:
