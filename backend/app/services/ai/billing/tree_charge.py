...
这条契约**两侧都是硬的**（终审 I1）：``persist_views()`` 返回它有没有落库，落不了
时 ``_finish`` **不调本函数**，把这棵树留给清扫器（提名条件是 ``charged_at IS NULL``，
戳没盖就还提得到）。在此之前镜像失败只打一条 WARNING 而收口照走 —— 整棵树按上一次
事件镜像的旧值结账，且戳一盖再也收不了第二次：一次写失败等于一笔永久漏收。
...
def cutover_at() -> Optional[datetime]:
    ...
    **为什么光有「这棵树扣过钱没有」那道正查不够**（2026-09-17 事故）：本机制上线后
    **头两轮**清扫（06:43 与 06:44:09），清扫器把 81 棵**上线前**的历史树
    （``ended_at`` 在 09-10~09-15）提名进来并整棵扣了 125 分。那些树结束于 A3（积分链修好）**之前** —— 那时一分钱都没扣过，
