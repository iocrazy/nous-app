...
#   NODE_TYPE_VIDEO_GEN = "video_gen"  → op "video_gen"  (StoryboardAIService.generate_video)
...
class ClassicDispatch:
    ...
    Two runnable kinds today:
      ...
      - ``kind="op"``       — a direct, already-implemented service call that is
        NOT a provider_slug. ``op`` names it (e.g. ``"image_gen"``). split /
        video_gen will each add one more op branch in ``resolve_classic_dispatch``
        plus one more handler in ``CanvasRunService.run_classic_node`` — no other
        wiring changes.
...
def resolve_classic_dispatch(
    ...
    NEXT TASK: add ``split`` and ``video_gen`` here as two more ``op`` branches
    (each returning ``kind="op"`` with the matching op id), then one handler
    apiece in ``CanvasRunService.run_classic_node``.
    """
    if node_type == NODE_TYPE_IMAGE_GEN:
        return ClassicDispatch(kind="op", op=OP_IMAGE_GEN)

    # Future op branches (split / video_gen) slot in right here.
