...
class BackfillStats:
    ...
    failed: int = 0
    #: 正文为空、**且**在 ``run_deliverables`` 里找不到登记行的产出行。也就是
    #: 「这次回填够不着的空行」。**不处理**，只报数 —— 两条候选集查询都是内连接，
    #: 这些行根本进不来，所以少了这个数，一次「全部补完」的汇总和一次「还漏着
    #: N 条」的汇总长得一模一样。
    #:
...
async def _orphan_output_count() -> int:
    """**正文为空、且** ``run_deliverables`` 里找不到登记行的产出行数。
    ...
    ⚠️ **「正文为空」这一半不能省。** 一条没有登记行但正文已经有了的投影行，对
    这份回填不构成任何欠账：它在检索里搜得到，回填也没有该做而没做的事。把它算
    进来，这个数就从「还欠着多少」退化成「历史上有多少行对不上登记表」—— 后者
    也是个真问题，但**不是这个汇总要回答的问题**，混在一起只会让运维盯着一个
    永远归不了零的数，然后不再看它。
...
async def backfill_search_docs_bodies(
    *, dry_run: bool = False, limit: Optional[int] = None
    ...
    async with system_request_scope("backfill search_docs bodies (3c)"):
        # 孤儿普查（回填够不着的空行）是个**装饰性**的数。它坏掉不该否决一次
        # 能干活的回填（同
