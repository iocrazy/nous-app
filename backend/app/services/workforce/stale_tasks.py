...
oldest first, at most ``limit`` per tick.
...
async def reap_stale_workforce_tasks(
    ...
    """One pass over stale workforce tasks. Returns per-outcome counts plus
    ``candidates`` and ``errors``; never raises."""
    ...
    try:
        tasks = await repo.list_stale_active_tasks(older_than=older_than, limit=limit)
...
async def _fail_worker_lost(
    ...
    if (payload.get("kind") or "") == "subagent" and parent_run_id:
        if not run_id:
            logger.warning(
                f"[stale-tasks] subagent task {task_id} has no child run; "
                f"parent {parent_run_id}'s async_pending cannot drain here "
                f"(forced settle will close the tree)"
            )
