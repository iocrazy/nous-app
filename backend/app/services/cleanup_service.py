...
class CleanupService:
    ...
    async def get_cleanup_data(
        ...
        try:
            ...
            categories["duplicate_content"] = 0  # Will be populated separately if needed
                ...
                try:
                    ...
                    for dup in duplicates:
                        if dup["video_id"] not in existing_ids:
                            ...
                            categories["duplicate_content"] = categories.get("duplicate_content", 0) + 1
            ...
            suggestions, categories = await self.get_suggestions(user_id, limit, include_duplicates)
    ...
    async def get_suggestions(
        ...
        try:
            ...
            main_suggestions = results[0] if not isinstance(results[0], Exception) else []
            if isinstance(results[0], Exception):
                logger.warning(f"RPC get_cleanup_suggestions failed, falling back: {results[0]}")
            ...
            return await self._get_suggestions_legacy(user_id, limit, include_duplicates)
            ...
            for v in results[1]:
                suggestions.append({
                    **v,
                    "video_id": v["id"],
                    "reason": "old_unused",
                    "reason_detail": f"Not viewed in over 30 days"
                })
