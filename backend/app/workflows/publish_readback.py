...
这是整条链最容易写错的地方，所以枚举写在这里而不是散在调用点：
  ...
  * ``VERIFIED``  —— 平台上真的看得到 → 待办可以 done（能拿到 id 就顺带回填
                     URL，见下方「关于 published_url 的丑话」）
...
关于 published_url 的丑话
=========================
P1-3 的原始描述是"回填 ``published_url`` / ``platform_item_id``"。**这一半在
抖音上大概率兑现不了**，而且不是实现没做到，是平台没给：
``frontend/components/Distribution/RecordsPage.tsx`` 里记着 2026-08-08 对着
真实账号的实测 —— 作品管理页的卡片**既没有 href、也没有 id 属性，列表 XHR
也不返回**（三处都查过）。这正是前端那颗按钮写"Open in platform"而不是
"View post"的原因。

所以本模块真正交付的是**结论**（上线 / 审核中 / 被拒 / 找不到），它从卡片的
状态文案读出来，根本不需要 id。URL 回填是**顺带**：拿得到 id 就写，拿不到就
不写（``verification_update_stmt`` 只在非空时才写这两列），**不影响 verdict**。

把这条写在这里而不是让它静静地不生效，是因为下一个人会拿"published_url 还是
空的"当 bug 来查。它不是 bug，是平台的能力边界。
