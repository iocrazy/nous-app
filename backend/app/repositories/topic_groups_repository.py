...
class TopicGroupRepository:
    async def list_unclustered(
        ...
        Returns ``id`` (text), ``source_id`` (text), ``title``, ``vec``
        (the embedding as a pgvector text literal) and ``embedding_model``
        (the vector's space, None = legacy). SQL body kept: the
            ...
            result = await session.execute(
                text(
                    ...
                    SELECT id::text AS id, source_id::text AS source_id,
                           title, embedding::text AS vec, embedding_model
    ...
    async def nearest_group(
        ...
        *,
        ...
        Only groups in ``vec``'s embedding space (``embedding_model``; NULL on
        either side = legacy, compatible) are candidates — a centroid from
        another model would score meaninglessly. Returns ``{"id": text, "sim":
        float}`` or None when no active group exists. ``sim`` is cosine
        similarity (1 - cosine distance). SQL body kept: ``<=>`` distance
        ordering is pgvector-native."""
        same_space = spaces_compatible_sql("embedding_model", "CAST(:model AS text)")
        async with read_scope() as session:
            row = (
                (
                    await session.execute(
                        ...
                        {"vec": vec, "win": window_hours, "model": embedding_model},
                    )
                )
                .mappings()
                .first()
    ...
    async def create_group(
        self, *, label: str, vec: str, embedding_model: Optional[str] = None
    ) -> Optional[str]:
        """Create a group seeded with this hotspot's centroid. Returns its id.

        ``embedding_model`` = the seed vector's space; the centroid IS that
        vector, so it inherits it.
                ...
                await session.execute(
                    text(
                        ...
                        INSERT INTO public.topic_groups
                            (label, embedding, embedding_model, source_count,
                             first_seen, last_seen)
                        VALUES (:label, CAST(:vec AS vector), :model, 1, now(), now())
                        RETURNING id::text
                        """
                    ),
                    {
