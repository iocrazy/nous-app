"""Notification repository — data access for notifications / user_notifications.

ORM 2.0 (post-rollout cleanup): ``NotificationRepository`` is the SQLAlchemy
2.0 ORM implementation for the notifications / user_notifications / team_members
surface. The legacy supabase-py REST path and its per-domain rollout flag have
been retired; call sites go through ``get_notification_repository()`` (bottom of
this file) which now unconditionally returns the ORM-backed repository.
...
LEGACY QUIRK PRESERVED — delete_notification:
  ...
  so its OBSERVABLE contract today is "graceful no-op returning False". This
  reproduces that EXACT contract (it cannot persist a phantom column either)
  rather than silently inventing new behavior. Fixing the dismiss feature is a
  logic change out of scope for a mechanical migration.
...
class NotificationRepository:
    """Repository for notification database operations (SQLAlchemy 2.0 ORM)."""
    ...
    async def get_user_notifications(
        ...
        Resolve the user's team memberships, then (system + team) notifications
        ordered newest-first, then join the per-user read status. bigint ids
        stay int so the in-Python ``n['team_id'] in team_ids`` filter and the
        id→read_status join keep matching."""
        try:
    ...
    async def delete_notification(self, notification_id: str, user_id: str) -> bool:
        ...
        the method returns False (a graceful no-op). We cannot persist a phantom
        column either, so we reproduce the EXACT observable contract — return
        False without mutating anything. Fixing the dismiss feature is a logic
        change out of scope for this mechanical migration."""
        logger.warning(
            "delete_notification is a legacy no-op (user_notifications has no "
            "dismissed_at column); returning False for notification {}",
            notification_id,
        )
        return False
...
def get_notification_repository() -> "NotificationRepository":
    """Return the NotificationRepository (SQLAlchemy 2.0 ORM).
