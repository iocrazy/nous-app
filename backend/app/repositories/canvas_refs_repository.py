...
two reads that JOINed ``resources`` (``list_assets_for_canvas`` /
``tree_for_projects``) were deleted with their routes in the P4 OpenAPI
typing pass (2026-09-24): no client called them. The table is RLS-locked
to service_role and the engine role (postgres) carries BYPASSRLS, so the session write path has the same
