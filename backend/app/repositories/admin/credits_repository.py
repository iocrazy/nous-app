"""Repository for admin credits/points management (ORM 2.0, model-backed — ★ MONEY ★).

ORM 2.0 (post-rollout cleanup): ``AdminCreditsRepository`` is the SQLAlchemy 2.0
implementation for the admin console's credit administration — system-wide stats,
transaction/order ledgers, package + pricing CRUD, per-team credit detail, and
the enrichment helpers that the grant/adjust/refund router flows lean on. The
legacy supabase-py REST path and its per-domain rollout flag
(``USE_ORM_ADMIN_CREDITS``) have been retired; call sites go through
``get_admin_credits_repository()`` (bottom of this file) which now unconditionally
returns this repository.

Scoped to package + pricing config operations plus stats/reads/order flows.
...
``app/api/admin/credits_router.py`` proving the chosen shape:
  ...
  ── point_transactions.duration_seconds (Numeric) → the ONLY Numeric column
     ...
     reproduction (matches points_repository's identical decision).
...
(_get_team_display_name reads ``team.get("is_personal")``) keep working. We
reproduce this derivation byte-for-byte: ``kind`` is a plain ``Text`` column
...
parity (the legacy used ``.select("id, name, kind, owner_id")``); we emit the
same 4-key (+ derived is_personal) dict.
...
— a SEPARATE repo (PointsRepository) owns that atomicity. This repo's only writes
are:
...
Error handling mirrors the legacy: it lets exceptions propagate (no defensive
try/except) on the CRUD + read methods, so behaviour is byte-identical to REST's
400/500 propagation that the router's 404 guards rely on. The two atomic RPC
helpers swallow + log + return None on RPC failure (engine/REST unavailable),
matching the legacy.
"""
...
class AdminCreditsRepository:
    """ORM-backed admin credits repository (model-backed reads + writes)."""
    ...
    async def confirm_order_and_credit_atomic(
        ...
        # Atomic, idempotent order-confirm-and-credit via the
        # rpc_confirm_order_and_credit SECURITY DEFINER function (mig 260) — the
        # SAME RPC the payment-callback path uses (no duplicated logic). Marks paid
        # + credits team_quotas + writes the ledger row in ONE transaction (closes
        # the two-await crash gap + double-credit risk). write_scope() owns the
        # commit (a read_scope would silently roll the credit back: lost money).
        # The TABLE return comes back via mappings() — INTEGER cols -> native int.
    ...
    async def refund_order_and_debit_atomic(
        ...
        # Atomic, idempotent order-REFUND-and-debit via the
        # rpc_refund_order_and_debit SECURITY DEFINER function (mig 261) — sibling
        # of confirm_order_and_credit_atomic (no duplicated logic). Marks refunded
        # + debits team_quotas (CLAMP AT 0) + writes the ledger row in ONE
        # transaction. write_scope() owns the commit (a read_scope would silently
        # roll the debit back). The TABLE return comes back via mappings() —
        # INTEGER cols -> native int. CLAMP AT 0 + idempotency live in the RPC.
        from loguru import logger
...
def get_admin_credits_repository() -> AdminCreditsRepository:
    """Return the admin credits repository (ORM-only, post-rollout).

    The per-domain rollout flag ``USE_ORM_ADMIN_CREDITS`` has been retired; this
    factory unconditionally returns the SQLAlchemy 2.0 ``AdminCreditsRepository``.
    """
