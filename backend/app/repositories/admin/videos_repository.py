...
ORM 2.0 (post-rollout cleanup): ``AdminVideosRepository`` is the SQLAlchemy 2.0
implementation for the admin video-management console, which reads (and, for
delete / retry-reset, WRITES) the ``parsed_media`` table. The legacy supabase-py
REST path and its per-domain rollout flag have been retired; call sites go through
``get_admin_videos_repository()`` (bottom of this file) which now unconditionally
...
class AdminVideosRepository:
    """ORM-backed admin video repository (parsed_media reads + delete/reset)."""
...
def get_admin_videos_repository() -> "AdminVideosRepository":
    """Return the AdminVideosRepository (SQLAlchemy 2.0 ORM).
