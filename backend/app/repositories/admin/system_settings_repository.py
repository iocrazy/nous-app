...
supabase-py REST path was retired with ``USE_ORM_ADMIN_SYSTEM_SETTINGS``).
Centralizing these calls here:
...
def get_system_settings_repository() -> "SystemSettingsRepository":
    """Return the SystemSettingsRepository (system_settings admin CRUD).

    Post-rollout this is unconditionally the SQLAlchemy 2.0 ORM implementation;
    the legacy supabase-py REST path and its ``USE_ORM_ADMIN_SYSTEM_SETTINGS``
