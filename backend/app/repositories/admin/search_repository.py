"""Repository for admin cross-log search + request tracing (SQLAlchemy 2.0 ORM).

Post-rollout the repository is the SQLAlchemy 2.0 implementation — every read
goes through ``read_scope()`` with a Core ``select()`` or a parameterized
``text()`` statement. There is no legacy supabase-py REST path anymore. Call
sites route through ``get_admin_search_repository()`` (bottom of this module).
...
======
  ...
  frontend_error_logs → ``app.models.FrontendErrorLogs``; the real column is
    ``stack`` (NOT ``stack_trace``). The projection selects ``stack``.
  audit_logs          → ``app.models.AuditLogs``; the admin column is ``admin_id``
    (uuid), NOT ``admin_email``. The table + column are corrected here.
...
endpoint 500'd. FIXED: the projection selects ``stack`` (and returns it under the
``stack`` result key). The router consumer never reads the stack field, so no
...
column error. FIXED: ``AUDIT_LOGS_TABLE`` points at ``audit_logs`` and the
...
READS ONLY — there are no writes in this repo. The ``frontend_logs`` / ``audit_logs``
``text()`` statements interpolate the ``*_TABLE`` class constants (fixed literals,
never user input) into the FROM clause; every value bound goes through named
parameters.
"""
...
def get_admin_search_repository() -> "AdminSearchRepository":
    """Return the AdminSearchRepository (SQLAlchemy 2.0 ORM, post-rollout).

    The per-domain rollout flag has been retired; there is a single ORM-backed
    implementation.
    """
