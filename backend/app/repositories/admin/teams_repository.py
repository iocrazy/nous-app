"""Repository for admin team management — SQLAlchemy 2.0 ORM.

ORM 2.0 (post-rollout cleanup): ``AdminTeamsRepository`` is the SQLAlchemy 2.0
implementation for the admin team-management console
(``app/api/admin/teams_router.py``), which reads + WRITES ``teams`` /
``team_members`` / ``team_quotas`` / ``collections``. The legacy supabase-py REST
path and its per-domain rollout flag (``USE_ORM_ADMIN_TEAMS``) have been retired;
call sites go through ``get_admin_teams_repository()`` (bottom of this file) which
...
Per-column evidence (from teams_router.py):
  ...
  - ``teams.id`` / ``team_members.team_id`` / ``team_quotas.team_id`` (BIGINT) →
    **native int**. DICT-KEY EVIDENCE: list_teams ``team_ids = [t["id"]]`` →
    ...
    int math). batch_points_balances keeps the ``str(team_id)`` key.
...
GET semantics: ``get`` / ``get_member`` return ``None`` on a 0-row result via
``scalars().one_or_none()`` — the legacy ``.single()`` RAISED on a missing row
(→ HTTP 500), making the routers' ``if not team`` / ``if not await repo.get_member``
None-guards (which raise a proper 404) effectively DEAD for the missing-row case.
Now the router None-guards fire and return 404 instead of 500. list-style + batch
reads keep the None/empty-on-absent contract.
...
class AdminTeamsRepository:
    """ORM-backed admin teams / members / quotas repository (reads + writes)."""
...
def get_admin_teams_repository() -> "AdminTeamsRepository":
    """Return the AdminTeamsRepository (SQLAlchemy 2.0 ORM).
