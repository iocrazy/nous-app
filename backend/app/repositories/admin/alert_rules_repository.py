"""Alert rules repository — data access for alert_rules + alert_history.

ORM 2.0 (post-rollout cleanup): ``AlertRulesRepository`` is the SQLAlchemy 2.0
implementation for the admin alert-rules + alert-history console (monitoring
notifications). The legacy supabase-py REST path and its per-domain rollout flag
have been retired; call sites go through ``get_alert_rules_repository()`` (bottom
...
package. Every statement uses bound parameters (``:name``) — no string
interpolation of user values — so there is no SQL-injection surface. The table
NAMES come from the trusted constants (RULES_TABLE / HISTORY_TABLE), not user
input.
...
ALL writes COMMIT via ``write_scope()``:
  create_rule → INSERT ... RETURNING * (binds only the columns the payload
    carries; id / created_at / updated_at use server defaults). Returns the full
  ...
  update_rule → stamps ``updated_at = now()`` (REPRODUCING the legacy, which always
    ...
    phantom key cannot reach SQL (graceful — a no-op key is dropped).
...
class AlertRulesRepository:
    """ORM-backed alert rules + history repository (text() over PG)."""
...
def get_alert_rules_repository() -> "AlertRulesRepository":
    """Return the AlertRulesRepository (SQLAlchemy 2.0 ORM).
