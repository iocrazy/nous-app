...
"""SQLAlchemy 2.0 ORM data access for the review system.

Handles CRUD for ``review_comments`` (top-level comments + threaded replies),
``review_annotations`` (drawing/marker data per comment), and ``review_status``
(per-resource/version/reviewer approval state). Post-rollout the ORM path is the
only path — the ``USE_ORM_REVIEW`` flag and the legacy supabase-py REST bodies
have been retired (prod ran 100% ORM). ``get_review_repository()`` unconditionally
...
omission risk) used by issue_repository.
...
def get_review_repository() -> ReviewRepository:
    """Return the ReviewRepository (ORM-backed; the USE_ORM_REVIEW flag is
    retired — prod runs 100% ORM)."""
