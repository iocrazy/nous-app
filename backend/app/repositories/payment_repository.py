...
Data access for the ``orders`` table. Post-rollout: prod runs 100% ORM, so the
per-domain ``USE_ORM_PAYMENT`` flag and the legacy supabase-py REST bodies have
been retired — ``PaymentRepository`` is now the single ORM-backed class and
``get_payment_repository()`` returns it unconditionally. Call sites
(payment_service.PaymentService) are zero-touch (same class name, same public
signatures).
...
parity; NEVER str a bigint, the 5.3 trap):
  ...
  id / team_id (BigInteger): order id + team FK. CONSUMER:
    ...
    the PointsRepository.create_transaction write boundary, since asyncpg's
...
update_order swallow + RE-RAISE; get_order_by_id / get_order_by_trade_no /
get_team_orders swallow + return None/[]; expire_pending_orders swallows +
returns 0.
"""
...
class PaymentRepository:
    """Payment Order Repository (async, ORM-backed)."""
    ...
    async def confirm_order_and_credit_atomic(
        ...
        # (mig 260) the REST path used. write_scope() owns the surrounding
        # transaction/commit (a read_scope here would silently roll the credit
        # back: lost money). p_order_id is a BIGINT function param -> int(). The
        # TABLE return (success/already_credited/points_added/new_balance/reason)
...
def get_payment_repository() -> PaymentRepository:
    """Return the payment repository (ORM-backed, post-rollout).

    The per-domain ``USE_ORM_PAYMENT`` flag has been retired — prod runs 100% ORM.
    Unconditionally returns the SQLAlchemy-backed ``PaymentRepository``.
    """
