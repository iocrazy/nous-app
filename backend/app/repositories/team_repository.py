...
class TeamRepository:
    ...
    async def get_user_teams(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        # Get team details
        teams = await client.table("teams").select("*").in_("id", team_ids).order("created_at", desc=True).execute()

        return teams.data or []

    async def get_team_by_id(self, team_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        """Get a team by ID if user has access."""
        client = await get_async_supabase_admin()

        # Check membership
        membership = await client.table("team_members").select("*").eq("team_id", team_id).eq("user_id", user_id).execute()

        if not membership.data:
            return None

        # Get team
        team = await client.table("teams").select("*").eq("id", team_id).single().execute()
    ...
    async def create_team(self, name: str, owner_id: str) -> Dict[str, Any]:
        ...
        # Create team
        team = await client.table("teams").insert({
            "name": name,
            "owner_id": owner_id
    ...
    async def update_team(
        ...
        **updates
        ...
        # Check ownership
        team = await client.table("teams").select("*").eq("id", team_id).eq("owner_id", user_id).execute()

        if not team.data:
            return None

        # Update team
        result = await client.table("teams").update(updates).eq("id", team_id).select().single().execute()
    ...
    async def delete_team(self, team_id: str, user_id: str) -> bool:
        ...
        # Check ownership
        team = await client.table("teams").select("id").eq("id", team_id).eq("owner_id", user_id).execute()
    ...
    async def get_team_members(self, team_id: str, user_id: str) -> List[Dict[str, Any]]:
        ...
        # Get all members
        members = await client.table("team_members").select("*").eq("team_id", team_id).order("joined_at").execute()
    ...
    async def add_member(
        ...
        try:
            result = await client.table("team_members").insert({
                "team_id": team_id,
                "user_id": new_user_id,
                "role": role
            }).select().single().execute()
    ...
    async def update_member_role(
        ...
        requester = await client.table("team_members").select("role").eq("team_id", team_id).eq("user_id", requester_id).execute()

        if not requester.data or requester.data[0]["role"] not in ["owner", "admin"]:
            return False

        # Update role
        await client.table("team_members").update({"role": role}).eq("team_id", team_id).eq("user_id", target_user_id).execute()
    ...
    async def remove_member(
        ...
        team = await client.table("teams").select("owner_id").eq("id", team_id).single().execute()
        if team.data and team.data["owner_id"] == target_user_id:
            return False

        # Remove member
        await client.table("team_members").delete().eq("team_id", team_id).eq("user_id", target_user_id).execute()

        return True

    async def get_team_by_invite_code(self, invite_code: str) -> Optional[Dict[str, Any]]:
        """Get a team by its invite code."""
        client = await get_async_supabase_admin()

        result = await client.table("teams").select("*").eq("invite_code", invite_code.upper()).execute()
