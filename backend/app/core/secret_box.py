...
Key management
==============
The Fernet key comes from env ``NOUS_TOKEN_ENCRYPTION_KEY`` (44-char
URL-safe base64; the legacy name ``MEDIAHUB_TOKEN_ENCRYPTION_KEY`` is still
accepted — see ``app.core.env_names`` for precedence). Generate one with:

    python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"

To rotate: deploy a new key as ``NOUS_TOKEN_ENCRYPTION_KEY`` AND
keep the old one as ``NOUS_TOKEN_ENCRYPTION_KEY_OLD``. The decryptor
...
either (a) set NOUS_TOKEN_ENCRYPTION_KEY in .env, or (b) use the
...
_KEY = "TOKEN_ENCRYPTION_KEY"
_KEY_OLD = "TOKEN_ENCRYPTION_KEY_OLD"
_NOT_SET = "{} (or legacy {}) not set".format(*env_names(_KEY))
...
def _resolve_keys(*, allow_dev_fallback: bool = True) -> list[bytes]:
    ...
    keys: list[bytes] = []
    primary = env_alias(_KEY)
    previous = env_alias(_KEY_OLD)
    if primary:
...
def encrypt(
    ...
    if f is None:
        raise SecretBoxNotConfigured(f"{_NOT_SET}; cannot encrypt secret")
...
def decrypt(
    ...
    f = _fernet(allow_dev_fallback=allow_dev_fallback)
    if f is None:
        raise SecretBoxNotConfigured(f"{_NOT_SET}; cannot decrypt secret")
    try:
