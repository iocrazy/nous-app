...
from app.schemas.nous_model import NousModelProbeRequest
...
async def test_probe_models_returns_provider_models():
    ...
    from app.api.admin.nous_model_router import probe_nous_models

    body = NousModelProbeRequest(
    ...
    with patch(
        "app.api.admin.nous_model_router.AIProviderFactory.test_connection",
...
async def test_probe_models_surfaces_failure():
    ...
    body = NousModelProbeRequest(provider_key="volcengine", api_key="k", app_id="a")
    fake_auth = MagicMock()
    fake_auth.user_id = "admin-1"
...
async def test_test_endpoint_persists_not_probed_not_fail():
    ...
    with patch(
        ...
        with patch(
            ...
            resp = await test_nous_model("11", MagicMock())
...
async def test_test_endpoint_defaults_not_probed_false():
    ...
    with patch(
        ...
        with patch(
            ...
            resp = await test_nous_model("3", MagicMock())
