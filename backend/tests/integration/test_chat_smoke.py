...
async def test_chat_full_pipeline_fires_all_side_effects() -> None:
    ...
    session_row = {
        ...
        "agent_slug": "smoke-agent", "agent_id": str(agent_id),
    ...
    def _make_table(name: str):
        ...
        if name == "ai_messages":
            ...
            def _ins(payload):
                ...
                ins_chain = MagicMock()
                ins_chain.execute = AsyncMock(return_value=MagicMock(
    ...
    class _CM:
        async def __aenter__(self_inner): return recorder
        async def __aexit__(self_inner, *a): return False

    fake_agent = {
        "id": str(agent_id), "slug": "smoke-agent", "model": "qwen-max",
    ...
    with patch(
        ...
        out = await svc.chat(
            session_id, user_id=user_id, content="smoke test"
        )
    ...
    assert any("session-memory-update" in n for n in bg_tasks), (
        f"session_memory updater not dispatched. bg_tasks={bg_tasks}"
    )
    assert any("commitment-harvest" in n for n in bg_tasks), (
