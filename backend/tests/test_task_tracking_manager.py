"""Unit tests for TaskTrackingManager pure helpers.
...
from app.services.task_tracking_manager import (
    TaskPhase,
    TaskTrackingManager,
    VALID_TRANSITIONS,
    _PHASE_TO_STATUS,
...
class TestMakeDedupKey:
    def test_format(self) -> None:
        assert (
            TaskTrackingManager.make_dedup_key("download", "7412345678901234")
            == "task:download:7412345678901234"
        )

    def test_different_types_produce_different_keys(self) -> None:
        a = TaskTrackingManager.make_dedup_key("parse", "x")
        b = TaskTrackingManager.make_dedup_key("download", "x")
        assert a != b
...
class TestClassifyError:
    def test_network_timeout(self) -> None:
        assert TaskTrackingManager.classify_error(Exception("connection timed out")) == "NETWORK_TIMEOUT"

    def test_resource_404(self) -> None:
        assert TaskTrackingManager.classify_error(Exception("HTTP 404 Not Found")) == "RESOURCE_404"

    def test_rate_limited(self) -> None:
        assert TaskTrackingManager.classify_error(Exception("429 too many requests")) == "RATE_LIMITED"

    def test_transcode_failed(self) -> None:
        assert TaskTrackingManager.classify_error(Exception("ffmpeg exited with code 1")) == "TRANSCODE_FAILED"

    def test_ai_quota_exceeded_on_ai_keyword(self) -> None:
        # 'openai' keyword falls through to AI_QUOTA_EXCEEDED branch
        assert (
            TaskTrackingManager.classify_error(Exception("openai something"))
            == "AI_QUOTA_EXCEEDED"
        )

    def test_ai_quota_exceeded_on_explicit_msg(self) -> None:
        assert (
            TaskTrackingManager.classify_error(Exception("ai quota exhausted"))
            == "AI_QUOTA_EXCEEDED"
        )

    def test_storage_full(self) -> None:
        assert TaskTrackingManager.classify_error(Exception("disk full")) == "STORAGE_FULL"

    def test_ai_storage_full_becomes_ai_quota(self) -> None:
        # "openai ... quota exceeded" → AI_QUOTA_EXCEEDED branch
        assert (
            TaskTrackingManager.classify_error(Exception("openai quota exceeded"))
            == "AI_QUOTA_EXCEEDED"
        )

    def test_unknown_fallback(self) -> None:
        assert TaskTrackingManager.classify_error(Exception("mystery")) == "UNKNOWN"
...
def test_singleton_is_task_tracking_manager_instance() -> None:
    mgr = get_task_manager()
    assert isinstance(mgr, TaskTrackingManager)
