...
@_skip
@respx.mock
async def test_sync_leaves_unlisted_rows_and_statuses_alone(orm_dsn, pg, engine_env):
    """End-to-end through the real repository (spec 2026-09-25 §3.4): a
    service missing from the list keeps its enabled row, and a listed one's
    ``ready`` is not written into ``last_test_status``."""
    from app.services.ai.nous_engine_sync import sync_engine_models
    ...
    before = {
        r["name"]: dict(r)
        for r in await pg.fetch(
            "SELECT name, is_enabled, last_test_status, updated_at "
            "FROM public.nous_models WHERE base_url = $1",
            base_url,
        )
    }
