...
# **查证过的**既有不一致，带理由。新漂移立刻转红；名单里的条目一旦不再漂移，
# `test_allowlist_entries_are_still_drifting` 会**失败并要求删掉它** —— 豁免
# 不许放馊。
#
...
async def test_every_declared_index_exists_in_the_database(live_indexes) -> None:
    ...
    offenders = [
        (name, where)
        for name, where in _missing(live_indexes)
        if name not in ALLOWED_INDEX_DRIFT
    ]
    assert offenders == [], (
...
async def test_every_declared_index_matches_its_columns(live_indexes) -> None:
    ...
    offenders = [
        (name, why)
        for name, why in _mismatched(live_indexes)
        if name not in ALLOWED_INDEX_DRIFT
    ]
...
async def test_allowlist_entries_are_still_drifting(live_indexes) -> None:
    ...
    一条既不缺失也不错位的名字还留在 ALLOWED_INDEX_DRIFT 里，说明它已经被修好
    了（或者被删掉了）—— 此时**必须删掉豁免**，否则下一次同名漂移会被这条早已
    失效的豁免静静吃掉。
    """
    drifting = {name for name, _ in _missing(live_indexes)} | {
        name for name, _ in _mismatched(live_indexes)
    }
    stale = sorted(set(ALLOWED_INDEX_DRIFT) - drifting)
    assert stale == [], f"这些豁免已经不漂移了，请从 ALLOWED_INDEX_DRIFT 删掉：{stale}"
