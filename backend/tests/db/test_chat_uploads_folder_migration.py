...
WHY THIS SHIPS SEPARATELY FROM THE BACKEND
──────────────────────────────────────────
This PR carries ONLY the migration. The backend that reads ``system_key`` must
already be live when it runs: the old backend matched the folder by the literal
name ``temp``, so the instant this UPDATE renames one to "Chat Uploads" that
backend stops finding it and creates a second ``temp`` folder beside it on the
next chat upload. ``run-migration.yml`` is a few seconds of psql while
``deploy-gpu.yml`` is a docker build, so "migration first" is the NORMAL order,
not a rare race — which is why ordering is enforced by merging separately
rather than by anything in this SQL.
