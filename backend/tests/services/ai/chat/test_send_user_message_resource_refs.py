...
def _make_fake_runner(captured: dict) -> Any:
    ...
    runner.resource_fetch_handler = None  # will be set by the wiring
    return runner
...
async def test_resource_ref_wiring_calls_resolver_and_registers_tool():
    ...
    # 3. runner.resource_fetch_handler was set (not None)
    assert runner.resource_fetch_handler is not None
