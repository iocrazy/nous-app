...
def test_prompted_resources_are_bounded_by_a_limit():
    compiled = _compile(
        PromptCatalogRepository()._prompted_resources_stmt(SCOPE, project_id=None)
    )
    # Unbounded, this SELECT materializes every prompted resource in the scope
    # on every /prompts call. The bound rides as a parameter, so the keyword
    # alone would also hold for LIMIT 5 — pin the value too.
    assert "LIMIT" in str(compiled)
    assert 2000 in compiled.params.values()


def test_prompted_resources_limit_is_the_callers_to_choose():
    compiled = _compile(
        PromptCatalogRepository()._prompted_resources_stmt(
            SCOPE, project_id=None, limit=7
        )
    )
    assert "LIMIT" in str(compiled) and 7 in compiled.params.values()
