...
class _FakeScorer:
    ...
    async def score_items(self, items, user_id=None):
        self.calls += 1
        # Score even indices only, to exercise partial enrichment.
        return {
            it["i"]: {
                "score": 0.7,
                "reason": "r",
...
async def test_score_unscored_batches_and_patches():
    rows = [
        {"id": str(n), "source_label": "S", "title": f"t{n}", "content_original": "c"}
        for n in range(5)
    ]
    hs = _FakeHotspots(rows)
    sc = _FakeScorer()
    out = await score_unscored_once(
        hotspots_repo=hs, scorer=sc, max_items=10, batch_size=2
    )
...
async def test_score_unscored_batch_failure_isolated():
    rows = [
        {"id": str(n), "source_label": "S", "title": "t", "content_original": "c"}
        for n in range(3)
    ]
    hs = _FakeHotspots(rows)
    ...
    out = await score_unscored_once(
        hotspots_repo=hs, scorer=_Boom(), max_items=10, batch_size=2
    )
