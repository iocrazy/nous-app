...
@pytest.mark.asyncio
async def test_create_forwards_timeline_anchor():
    """A memo pin carries anchor_script_id + anchor_sec through to the service."""
    svc = AsyncMock()
    svc.create_note.return_value = {
        "id": 1,
        "content_md": "idea",
    ...
    with patch("app.api.inspiration_router.get_notes_service", return_value=svc):
        out = await create_note(
            NoteCreateIn(content_md="idea", anchor_script_id="555", anchor_sec=90),
            current_user=USER,
        )
    _, kwargs = svc.create_note.call_args
    assert kwargs["anchor_script_id"] == "555"
    assert kwargs["anchor_sec"] == 90
    assert out.anchor_sec == 90
    assert out.anchor_script_id == "555"


@pytest.mark.asyncio
async def test_update_absent_anchor_is_untouched():
    """A pinned-only PATCH must NOT touch the anchor columns — the repo reads the
    _UNSET sentinel and leaves them alone (a bare null would wrongly un-pin)."""
    from app.repositories.inspiration_repository import _UNSET

    svc = AsyncMock()
    svc.update_note.return_value = {
        "id": 9,
        "content_md": "x",
        "tags": [],
        "note_date": "2026-07-19",
    ...
    _, kwargs = svc.update_note.call_args
    assert kwargs["anchor_script_id"] is _UNSET
    assert kwargs["anchor_sec"] is _UNSET


@pytest.mark.asyncio
async def test_update_explicit_null_anchor_clears():
    """An explicit null anchor (un-pin) forwards None, distinct from _UNSET."""
    svc = AsyncMock()
    svc.update_note.return_value = {
        "id": 9,
        "content_md": "x",
    ...
    with patch("app.api.inspiration_router.get_notes_service", return_value=svc):
        await update_note(
            "9",
            NoteUpdateIn(anchor_script_id=None, anchor_sec=None),
            current_user=USER,
        )
    _, kwargs = svc.update_note.call_args
    assert kwargs["anchor_script_id"] is None
    assert kwargs["anchor_sec"] is None


@pytest.mark.asyncio
async def test_update_moves_anchor_sec():
    """Dragging a pin PATCHes a new anchor_sec value straight through."""
    svc = AsyncMock()
    svc.update_note.return_value = {
        "id": 9,
        "content_md": "x",
    ...
    with patch("app.api.inspiration_router.get_notes_service", return_value=svc):
        out = await update_note("9", NoteUpdateIn(anchor_sec=120), current_user=USER)
    _, kwargs = svc.update_note.call_args
    assert kwargs["anchor_sec"] == 120
    assert out.anchor_sec == 120


@pytest.mark.asyncio
async def test_anchor_sec_rejects_out_of_int4_range():
    """anchor_sec beyond PG INT4 max is rejected at the schema boundary."""
    import pydantic

    with pytest.raises(pydantic.ValidationError):
        NoteCreateIn(content_md="x", anchor_sec=2_147_483_648)
