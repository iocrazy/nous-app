...
def main() -> int:
    ...
    with psycopg.connect(admin_dsn(), autocommit=True) as conn:
        conn.execute(
            "DELETE FROM public.issues WHERE title LIKE '%PR-D1 verify%' OR title='XOR test' OR title='Test issue from PR-D1 verify'"
        )
    ...
    with psycopg.connect(admin_dsn()) as conn:
        ...
        for required in (
            ...
            if not ok:
                fails.append(f"missing column {required}")
        cur = conn.execute(
            "SELECT count(*) FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='issues'"
        )
        assert_eq("issues in supabase_realtime", cur.fetchone()[0], 1, fails)
        cur = conn.execute(
            "SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND tablename='issues'"
        )
    ...
    with psycopg.connect(admin_dsn(), autocommit=True) as conn:
        ...
        assert_eq(
            "identifier format", [r[1] for r in ids], ["MH-1", "MH-2", "MH-3"], fails
        )
    ...
    with psycopg.connect(admin_dsn(), autocommit=True) as conn:
        ...
        for tbl in ("unified_tasks", "agent_tasks", "project_tasks"):
            ...
            if not ok:
                fails.append(f"{tbl}.issue_id missing")
        cur = conn.execute(
            "SELECT count(*) FROM public.issues WHERE identifier=%s", (ident,)
        )
    ...
    with psycopg.connect(admin_dsn()) as conn:
        with conn.transaction():
            ...
            cur = conn.execute(
                "SELECT count(*) FROM public.issues WHERE created_by_user_id=%s",
                (USER_A,),
            )
        ...
        with conn.transaction():
            conn.execute("SET LOCAL ROLE authenticated")
            conn.execute(
                f'SET LOCAL request.jwt.claims = \'{{"sub":"{USER_B}","role":"authenticated"}}\''
            )
            cur = conn.execute(
                "SELECT count(*) FROM public.issues WHERE created_by_user_id=%s",
                (USER_A,),
    ...
    with psycopg.connect(admin_dsn(), autocommit=True) as conn:
        cur = conn.execute(
            "SELECT count(*) FROM public.issues WHERE title LIKE '%PR-D1 verify%' OR title='XOR test'"
        )
        n_test_rows = cur.fetchone()[0]
        conn.execute(
            "DELETE FROM public.issues WHERE title LIKE '%PR-D1 verify%' OR title='XOR test'"
        )
