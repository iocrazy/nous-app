...
- **两条出图链共用同一个二进制，区别在凭证、计费与真正被兑现的旋钮**：订阅行（`codex-image` / `codex-local-image`）的图像模型版本由 OpenAI 决定，quality 只到 `high`（`xhigh` 实测被改写成 `medium` 且不告知）、`--size` 被忽略；API-key 行（`openai-image-flare` / `-sunburst`，`openai-images` 协议）走 2.5 模型，认 `xhigh`/`max` 与精确尺寸，key 放在 Admin → AI Models 的行里、按 token 计费。看着像一个产品，别把两者的结论互相套用；详见 [`docs/runbook/codex-image.md`](docs/runbook/codex-image.md)。
