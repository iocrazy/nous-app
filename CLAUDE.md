...
- **向量链路三处静默：列维度、`text()` 的 bind、hybrid 的降级（2026-09-16 血泪）**：① ORM 写 `Vector(1536)` 而生产是 `vector(2048)`，pgvector 在 **bind 时**校验维度，于是每次写 `resource_analysis.content_embedding` 都抛 ValueError 并被上层当成「没向量，跳过」——analyze_l1 一路报成功，向量列零行躺了几个月。换 embedding 模型必须同批改 `app/models/media.py` 的 `Vector(n)`：只改库不改 ORM = 静默关掉写入。② `text()` 里**不能**写 `:t::double precision`——SQLAlchemy 的 bind 正则遇到名字后面跟 `::` 就不认，那两个参数变成字面量文本，`match_videos_by_embedding` 第一次收到真向量时才炸；一律 `CAST(:t AS double precision)`。③ hybrid 搜索的向量腿是 best-effort：embedder 未配置 / 超 `HYBRID_EMBED_TIMEOUT_S`（5s）/ RPC 缺失都降级成纯文本，而降级后的响应与健康时**逐字节相同**。所以有 `SearchResponse.vector_leg`（取值见 `app/services/library/search_service.py` 的 `VECTOR_LEG_OUTCOMES`）——读它，别把「那条腿没跑」读成「没匹配到」。④ 非对称检索：指令前缀（`QUERY_INSTRUCTION`）**只加在查询上**，文档保持原文。给文档侧也加前缀会掉召回，而改前缀本身不需要重嵌文档。
