...
- **生产错误体是 `ErrorResponse` 外壳，不是 FastAPI 裸 `{detail}`（2026-09-09 真栈）**：`app/core/exceptions.py` 把每个 `HTTPException` 包成 `{"success": false, "error": <文案>, "code": "http_<status>", "request_id": …, "details": <exc.detail 是 dict 时原样，否则 null>}`。前端解析类型化错误码要读 `details.code`（`apiClient.ts` 早就这么做），只读 `detail` 的解析器（2a 的 `_controlJson`、2b-1 的 `forkJson` 曾如此）在单测里因为用了 FastAPI 裸形状全绿，真栈上每个拒绝都退化成 `http_409` + "409 Conflict"。给这类解析器写测试时把 cn.nous.ink 的**原样响应体**贴进去（同「边界 mock 必须用真实 JSON 形状」）。
...
- **两条出图链共用同一个二进制，区别在凭证、计费与真正被兑现的旋钮**：服务端订阅行 `codex-image`（`codex` 协议）与 Admin 上的 Codex 登录卡已于 2026-09-23 下线（mig 498），现在只剩本机 daemon 行 `codex-local-image`（`codex-local` 协议，用户自己的订阅）与 API-key 行（`openai-images`）。订阅路径的图像模型版本由 OpenAI 决定，quality 只到 `high`（`xhigh` 实测被改写成 `medium` 且不告知）、`--size` 被忽略；API-key 行（`openai-image-flare` / `-sunburst`，`openai-images` 协议）走 2.5 模型，按官方文档认 `xhigh`/`max` 与精确尺寸（2026-09-13 尚未真栈实测），key 放在 Admin → AI Models 的行里、按 token 计费。看着像一个产品，别把两者的结论互相套用；详见 [`docs/runbook/codex-image.md`](docs/runbook/codex-image.md)。
- **向量链路三处静默：列维度、`text()` 的 bind、hybrid 的降级（2026-09-16 血泪）**：① ORM 曾写 `Vector(1536)` 而生产列是 `vector(2048)`；pgvector 的 SQLAlchemy 类型在 **bind 时**校验维度，维度对不上就是 ValueError，而调用方把它读成「没向量，跳过」。换 embedding 模型必须同批改 `app/models/media.py` 的 `Vector(n)`：只改库不改 ORM = 静默关掉写入。**2026-09-22 起（mig 490）**宽度与「当前空间」只在 `app/core/embedding_space.py` 一处：四个 ORM `Vector(EMBEDDING_DIM)` 读它，`EmbeddingService` 在写入口把宽度不符拦成类型化的 `dimension_mismatch`（不再是「没向量，跳过」）；每个向量旁有 `embedding_model`（实际 provider 模型 id），读方只比同空间，NULL = 490 之前的旧向量、视作当前空间——**换模型前先把 NULL 行盖上旧模型 id**（SQL 在该模块 docstring）。（2026-09-16 查时 `resource_analysis` 只有 2 行、0 条向量，语义层一直是空的，回填端点就是为此加的。）② `text()` 里**不能**写 `:t::double precision`——SQLAlchemy 的 bind 正则不认名字后面紧跟 `::` 的写法，`:t` / `:c` 原样当字面量发给 PG，`match_videos_by_embedding` 一调就报语法错（Codex 复核确认）；一律 `CAST(:t AS double precision)`。③ hybrid 搜索的向量腿是 best-effort：embedder 未配置 / 超 `HYBRID_EMBED_TIMEOUT_S`（5s）/ RPC 缺失都降级成纯文本，而降级后的响应与健康时**逐字节相同**。所以有 `SearchResponse.vector_leg`（取值见 `app/services/library/search_service.py` 的 `VECTOR_LEG_OUTCOMES`）——读它，别把「那条腿没跑」读成「没匹配到」。④ 非对称检索：指令前缀（`QUERY_INSTRUCTION`）**只加在查询上**，文档保持原文。给文档侧也加前缀会掉召回，而改前缀本身不需要重嵌文档。**2026-09-23 起（mig 499）语义层向量在 `resource_embeddings`（halfvec(2048) + HNSW，按 `space_id` 分空间）；`resource_analysis.content_embedding` 只读回退、下一版删。换模型 = 新空间 + 全量重嵌（回填端点就地嵌，不再派 VLM）。** 迁移未跑时写入口报类型化 `store_missing`、三条读路径回退旧列、两边都没有时 hybrid 的 `vector_leg` 是 `store_missing`；覆盖率看 `GET /api/v1/search/vectors/status`。
...
- **托管 runner 依赖账户付款正常**（**2026-09-07 复查：已恢复正常，`ci.yml` 已迁回托管**）。付款失败时所有 `ubuntu-latest` job 会在 2 秒内 failure 且**零步骤执行**（`runner_name` 为空），annotation 里写着 `recent account payments have failed`。此时 `CI`/`actionlint`/`pr-behind-check` 全红、前端链也发不出去，但 **self-hosted 的后端链不受影响**（不计费）。
  ⚠️ **分钟数计不计费由可见性决定，而它会来回切**（2026-09-09 查时是 public；private + Free 是 2000 分钟/月，也不支持 branch protection）。**别信"每次 PR 约 13–16 分钟"这类按墙钟的估算 —— 托管 runner 按 job 并行数计费**：五个 job 全跑一次实测 22–44 计费分钟（成功 run 平均 34：frontend 22 / backend 6.5 / browser 3 / rust 1.3 / codex 1）。按 2026-09-07~09 的 PR 频率折算 ≈ 17,000 分钟/月，private 下是额度的 8 倍。8 月 CI 还在 gpupc 时同口径也有 ≈ 10,000/月 —— 82% 是 `Verify Frontend Deploy` 指着死域名每次烧满 15 分钟（已修）。**所以「8 月没超额」靠的是 public，不是 self-hosted**；把 CI 搬回 gpupc 省的是分钟数不是墙钟（单 runner 串行排过 3.5 小时）。
  2026-09-09 起 `ci.yml` 按区域跑：`changes` job 用 REST 拿 PR 文件清单，`scripts/ci-changed-areas.sh` 映射到五个 job（映射表与三条不变量在脚本头部；兜底**只许**退化成全跑，`ci-changed-areas.selftest.sh` 每次先跑钉住这个方向）。按最近 60 个 PR 的改动面分布（25% 全跑 / 25% 只 backend / 20% 纯文档 / 18% 只 frontend）估算省一半。剩下的大头是 frontend job 的 vitest —— 928s 里 jsdom `environment` 占 388s、`import` 192s、真正跑测试只 205s —— 那是下一手，但它是测试基建改动，得先本机量过再动。
